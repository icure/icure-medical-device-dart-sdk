...
class CodingReference {
  ...
  /// Returns a new [CodingReference] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static CodingReference? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "CodingReference[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "CodingReference[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return CodingReference(
  ...
  static List<CodingReference>? listFromJson(
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = CodingReference.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, CodingReference> mapFromJson(dynamic json) {
    final map = <String, CodingReference>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = CodingReference.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
